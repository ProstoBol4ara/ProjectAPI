...
class Reviews(Base):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ["content_id"], ["content.content_id"], name="reviews_content_id_fkey"
        ),
        ForeignKeyConstraint(
            ["created_by"], ["users.user_id"], name="reviews_created_by_fkey"
        ),
        ForeignKeyConstraint(
            ["deleted_by"], ["users.user_id"], name="reviews_deleted_by_fkey"
        ),
        ForeignKeyConstraint(
            ["user_id"], ["users.user_id"], name="reviews_user_id_fkey"
        ),
    ...
    users: Mapped[Optional["Users"]] = relationship(
        "Users", foreign_keys=[created_by], back_populates="reviews"
    )
    users_: Mapped[Optional["Users"]] = relationship(
        "Users", foreign_keys=[deleted_by], back_populates="reviews_"
    )
    user: Mapped[Optional["Users"]] = relationship(
        "Users", foreign_keys=[user_id], back_populates="reviews1"
    )
