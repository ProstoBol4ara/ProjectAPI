...
class Directors(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["created_by"], ["users.user_id"], name="directors_created_by_fkey"
        ),
        ForeignKeyConstraint(
            ["deleted_by"], ["users.user_id"], name="directors_deleted_by_fkey"
        ),
    ...
    users: Mapped[Optional["Users"]] = relationship(
        "Users", foreign_keys=[created_by], back_populates="directors"
    )
    users_: Mapped[Optional["Users"]] = relationship(
        "Users", foreign_keys=[deleted_by], back_populates="directors_"
    )
    content_directors: Mapped[List["ContentDirectors"]] = relationship(
        "ContentDirectors", uselist=True, back_populates="director"
    )
