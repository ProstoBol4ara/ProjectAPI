...
class ContentActors(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["actor_id"], ["actors.actor_id"], name="content_actors_actor_id_fkey"
        ),
        ForeignKeyConstraint(
            ["content_id"],
            ["content.content_id"],
            name="content_actors_content_id_fkey",
        ),
        ForeignKeyConstraint(
            ["created_by"], ["users.user_id"], name="content_actors_created_by_fkey"
        ),
        ForeignKeyConstraint(
            ["deleted_by"], ["users.user_id"], name="content_actors_deleted_by_fkey"
        ),
    ...
    actor: Mapped[Optional["Actors"]] = relationship(
        "Actors", back_populates="content_actors"
    )
    content: Mapped[Optional["Content"]] = relationship(
        "Content", back_populates="content_actors"
    )
    users: Mapped[Optional["Users"]] = relationship(
        "Users", foreign_keys=[created_by], back_populates="content_actors"
    )
    users_: Mapped[Optional["Users"]] = relationship(
        "Users", foreign_keys=[deleted_by], back_populates="content_actors_"
    )
