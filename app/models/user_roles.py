...
class UserRoles(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["created_by"], ["users.user_id"], name="user_roles_created_by_fkey"
        ),
        ForeignKeyConstraint(
            ["deleted_by"], ["users.user_id"], name="user_roles_deleted_by_fkey"
        ),
        ForeignKeyConstraint(
    ...
    users: Mapped[Optional["Users"]] = relationship(
        "Users", foreign_keys=[created_by], back_populates="user_roles"
    )
    users_: Mapped[Optional["Users"]] = relationship(
        "Users", foreign_keys=[deleted_by], back_populates="user_roles_"
    )
    role: Mapped[Optional["Roles"]] = relationship("Roles", back_populates="user_roles")
    user: Mapped[Optional["Users"]] = relationship(
        "Users", foreign_keys=[user_id], back_populates="user_roles1"
    )
