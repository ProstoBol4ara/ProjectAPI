...
class UserSubscriptions(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["created_by"], ["users.user_id"], name="user_subscriptions_created_by_fkey"
        ),
        ForeignKeyConstraint(
            ["deleted_by"], ["users.user_id"], name="user_subscriptions_deleted_by_fkey"
        ),
        ForeignKeyConstraint(
            ["plan_id"],
        ...
        ForeignKeyConstraint(
            ["user_id"], ["users.user_id"], name="user_subscriptions_user_id_fkey"
        ),
    ...
    users: Mapped[Optional["Users"]] = relationship(
        "Users", foreign_keys=[created_by], back_populates="user_subscriptions"
    )
    users_: Mapped[Optional["Users"]] = relationship(
        "Users", foreign_keys=[deleted_by], back_populates="user_subscriptions_"
    )
    plan: Mapped[Optional["SubscriptionPlans"]] = relationship(
        "SubscriptionPlans", back_populates="user_subscriptions"
    )
    user: Mapped[Optional["Users"]] = relationship(
        "Users", foreign_keys=[user_id], back_populates="user_subscriptions1"
    )
