...
class ContentCountries(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["content_id"],
            ["content.content_id"],
            name="content_countries_content_id_fkey",
        ),
        ForeignKeyConstraint(
            ["country_id"],
            ["countries.country_id"],
            name="content_countries_country_id_fkey",
        ),
        ForeignKeyConstraint(
            ["created_by"], ["users.user_id"], name="content_countries_created_by_fkey"
        ),
        ForeignKeyConstraint(
            ["deleted_by"], ["users.user_id"], name="content_countries_deleted_by_fkey"
        ),
    ...
    content: Mapped[Optional["Content"]] = relationship(
        "Content", back_populates="content_countries"
    )
    country: Mapped[Optional["Countries"]] = relationship(
        "Countries", back_populates="content_countries"
    )
    users: Mapped[Optional["Users"]] = relationship(
        "Users", foreign_keys=[created_by], back_populates="content_countries"
    )
    users_: Mapped[Optional["Users"]] = relationship(
        "Users", foreign_keys=[deleted_by], back_populates="content_countries_"
    )
