...
class PaymentMethods(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "method_type::text = ANY (ARRAY['Credit Card'::character varying, 'Bank Transfer'::character varying, 'SBP'::character varying]::text[])",
            name="payment_methods_method_type_check",
        ),
        ForeignKeyConstraint(
            ["created_by"], ["users.user_id"], name="payment_methods_created_by_fkey"
        ),
        ForeignKeyConstraint(
            ["deleted_by"], ["users.user_id"], name="payment_methods_deleted_by_fkey"
        ),
        ForeignKeyConstraint(
            ["user_id"], ["users.user_id"], name="payment_methods_user_id_fkey"
        ),
    ...
    users: Mapped[Optional["Users"]] = relationship(
        "Users", foreign_keys=[created_by], back_populates="payment_methods"
    )
    users_: Mapped[Optional["Users"]] = relationship(
        "Users", foreign_keys=[deleted_by], back_populates="payment_methods_"
    )
    user: Mapped[Optional["Users"]] = relationship(
        "Users", foreign_keys=[user_id], back_populates="payment_methods1"
    )
    payments: Mapped[List["Payments"]] = relationship(
        "Payments", uselist=True, back_populates="payment_method"
    )
