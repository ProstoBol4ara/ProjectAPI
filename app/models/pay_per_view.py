...
class PayPerView(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["content_id"], ["content.content_id"], name="pay_per_view_content_id_fkey"
        ),
        ForeignKeyConstraint(
            ["created_by"], ["users.user_id"], name="pay_per_view_created_by_fkey"
        ),
        ForeignKeyConstraint(
            ["deleted_by"], ["users.user_id"], name="pay_per_view_deleted_by_fkey"
        ),
    ...
    content: Mapped[Optional["Content"]] = relationship(
        "Content", back_populates="pay_per_view"
    )
    users: Mapped[Optional["Users"]] = relationship(
        "Users", foreign_keys=[created_by], back_populates="pay_per_view"
    )
    users_: Mapped[Optional["Users"]] = relationship(
        "Users", foreign_keys=[deleted_by], back_populates="pay_per_view_"
    )
    payments: Mapped[List["Payments"]] = relationship(
        "Payments", uselist=True, back_populates="pay_per_view"
    )
