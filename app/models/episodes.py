...
class Episodes(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["content_id"], ["content.content_id"], name="episodes_content_id_fkey"
        ),
        ForeignKeyConstraint(
            ["created_by"], ["users.user_id"], name="episodes_created_by_fkey"
        ),
        ForeignKeyConstraint(
            ["deleted_by"], ["users.user_id"], name="episodes_deleted_by_fkey"
        ),
    ...
    content: Mapped[Optional["Content"]] = relationship(
        "Content", back_populates="episodes"
    )
    users: Mapped[Optional["Users"]] = relationship(
        "Users", foreign_keys=[created_by], back_populates="episodes"
    )
    users_: Mapped[Optional["Users"]] = relationship(
        "Users", foreign_keys=[deleted_by], back_populates="episodes_"
    )
