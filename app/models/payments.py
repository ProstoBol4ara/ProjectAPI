...
class Payments(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["created_by"], ["users.user_id"], name="payments_created_by_fkey"
        ),
        ForeignKeyConstraint(
            ["deleted_by"], ["users.user_id"], name="payments_deleted_by_fkey"
        ),
        ForeignKeyConstraint(
            ["pay_per_view_id"],
            ["pay_per_view.pay_per_view_id"],
            name="payments_pay_per_view_id_fkey",
        ),
        ForeignKeyConstraint(
            ["payment_method_id"],
    ...
    users: Mapped[Optional["Users"]] = relationship(
        "Users", foreign_keys=[created_by], back_populates="payments"
    )
    users_: Mapped[Optional["Users"]] = relationship(
        "Users", foreign_keys=[deleted_by], back_populates="payments_"
    )
    pay_per_view: Mapped[Optional["PayPerView"]] = relationship(
        "PayPerView", back_populates="payments"
    )
    payment_method: Mapped[Optional["PaymentMethods"]] = relationship(
