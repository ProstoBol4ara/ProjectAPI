...
class SubscriptionPlans(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["created_by"], ["users.user_id"], name="subscription_plans_created_by_fkey"
        ),
        ForeignKeyConstraint(
            ["deleted_by"], ["users.user_id"], name="subscription_plans_deleted_by_fkey"
        ),
    ...
    users: Mapped[Optional["Users"]] = relationship(
        "Users", foreign_keys=[created_by], back_populates="subscription_plans"
    )
    users_: Mapped[Optional["Users"]] = relationship(
        "Users", foreign_keys=[deleted_by], back_populates="subscription_plans_"
    )
    user_subscriptions: Mapped[List["UserSubscriptions"]] = relationship(
        "UserSubscriptions", uselist=True, back_populates="plan"
    )
