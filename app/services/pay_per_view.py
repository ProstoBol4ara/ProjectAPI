...
class PayPerViewService:
    ...
    async def get_pay_per_views(self):
        ...
        return None if pay_per_views is None else \
            [{"pay_per_view_id": pay_per_view.pay_per_view_id, "amount": pay_per_view.amount,
              "content_id": pay_per_view.content_id} for pay_per_view in pay_per_views]
    ...
    async def get_pay_per_view(self, pay_per_view_id: int):
        ...
        if not pay_per_view: raise ValueError("Pay per view not found")

        return {"pay_per_view_id": pay_per_view.pay_per_view_id, "amount": pay_per_view.amount,
    ...
    async def create_pay_per_view(self, amount: float, content_id: float):
        ...
        return {"pay_per_view_id": new_pay_per_view.pay_per_view_id, "amount": new_pay_per_view.amount,
    ...
    async def update_pay_per_view(self, pay_per_view_id: int, amount: float = None, content_id: float = None):
        pay_per_view = await self.pay_per_view_repository\
            .update_pay_per_view(pay_per_view_id=pay_per_view_id, amount=amount, content_id=content_id)
    ...
    async def delete_pay_per_view(self, pay_per_view_id: int):
        if not await self.pay_per_view_repository.delete_pay_per_view(pay_per_view_id=pay_per_view_id):
