...
class DirectorsService:
    ...
    async def get_directors(self):
        ...
        return None if directors is None else [{"director_id": director.director_id, "director_name": director.director_name, "biography": director.biography, "birth_date": director.birth_date} for director in directors]

    async def get_director(self, director_id: int):
        director = await self.directors_repository.get_director(director_id=director_id)
        return None if director is None else {"director_id": director.director_id, "director_name": director.director_name, "biography": director.biography, "birth_date": director.birth_date}

    async def create_director(self, director_name: str, biography: str = None, birth_date: str = None):
        new_director = await self.directors_repository.create_director(director_name=director_name, biography=biography, birth_date=birth_date)
    ...
    async def update_director(self, director_id: int, director_name: str = None, biography:str = None, birth_date: str = None):
        director = await self.directors_repository.update_director(director_id=director_id, director_name=director_name, biography=biography, birth_date=birth_date)
        return None if director is None else {"director_id": director.director_id, "director_name": director.director_name, "biography": director.biography, "birth_date": director.birth_date}
